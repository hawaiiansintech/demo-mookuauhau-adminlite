...
<div>Uploading a GEDCOM file will queue it for processing to a new Moʻokūʻauhau (family tree) dataset.</div>
...
{#if $isSignedIn && $user}
    ...
    <form on:submit|preventDefault={uploadFormSubmit}>
        ...
        <div class="chan" on:click={()=>{fileinput.click();}}>Choose GEDCOM file</div>
        <input name="gedcomfile" type="file" accept=".ged, .gedcom" on:change={(e) => onFileSelected(e) } bind:this={fileinput} >
